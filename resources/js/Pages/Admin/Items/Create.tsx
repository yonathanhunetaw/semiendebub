...
export default function Create({ categories, colors, sizes, packagingTypes }: any) {
    // 1. Added 'errors' and 'status' to the form hook to satisfy Controller validation[cite: 1]
    const { data, setData, post, processing, errors } = useForm({
        ...
        images: [] as File[],
        status: "active", // Added status[cite: 1]
    });
    ...
    // 2. Updated inputStyle to be theme-aware
    const inputStyle = {
            ...
            <Box sx={{ mt: 1 }}>
                {activeCreator !== uniqueId ? (
                    ...
                    <Stack direction="row" spacing={1} sx={{ mt: 1 }}>
                        ...
                        <Button size="small" onClick={() => setActiveCreator(null)}>Cancel</Button>
                    </Stack>
                )}
            </Box>
        );
    ...
    return (
        // 3. Changed bgcolor and color to theme-aware variables to fix light/dark mode issue
        <Box sx={{ p: 3, bgcolor: 'background.default', minHeight: '100vh', color: 'text.primary' }}>
            ...
            {/* 4. Updated Paper to use theme background */}
                ...
                <form onSubmit={(e) => { e.preventDefault(); post(route('admin.items.store')); }}>
                    <Grid container spacing={4}>
                        <Grid size={{ xs: 12, md: 7 }}>
                            <Stack spacing={4}>
                                <TextField fullWidth label="Product Name" value={data.product_name} onChange={e => setData('product_name', e.target.value)} sx={inputStyle} error={!!errors.product_name} helperText={errors.product_name} />
                                <TextField fullWidth multiline rows={3} label="Product Description" value={data.product_description} onChange={e => setData('product_description', e.target.value)} sx={inputStyle} error={!!errors.product_description} helperText={errors.product_description} />
                                ...
                                <Box>
                                    <Autocomplete fullWidth freeSolo options={categories}
                                        ...
                                        renderInput={(p) => <TextField {...p} label="Category" sx={inputStyle} error={!!errors.item_category_id} helperText={errors.item_category_id} />}
                                    />
                        ...
                        <Grid size={{ xs: 12, md: 5 }}>
                            <Stack spacing={3}>
                                ...
                                {data.packaging.map((row, i) => (
                                    <Box key={i} sx={{ mb: 3, p: 2, borderRadius: 1, border: '1px solid', borderColor: 'divider', bgcolor: 'action.hover' }}>
                                ...
                                <Divider sx={{ my: 2 }} />
                                <Box sx={{ border: '2px dashed', borderColor: 'divider', borderRadius: 2, p: 3, textAlign: 'center', cursor: 'pointer', '&:hover': { borderColor: 'primary.main' } }} component="label">
                                    <input type="file" hidden multiple accept="image/*" onChange={e => setData('images', [...data.images, ...Array.from(e.target.files || [])])} />
                                    <CloudUploadIcon sx={{ fontSize: 35, color: 'text.secondary', mb: 1 }} />
                    ...
                    {/* 5. Added Validation Error Display banner */}
                    {Object.keys(errors).length > 0 && (
                        <Box sx={{ mt: 4, mb: -2 }}>
                            <Alert severity="error" variant="filled" sx={{ borderRadius: 2 }}>
                                <Typography variant="subtitle2" fontWeight="bold">Validation Failed:</Typography>
                                <ul style={{ margin: 0, paddingLeft: '20px' }}>
                                    {Object.entries(errors).map(([field, message]) => (
                                        <li key={field} style={{ fontSize: '0.85rem' }}>
                                            <strong>{field.replace('_', ' ')}:</strong> {message as string}
                                        </li>
                                    ))}
                                </ul>
                            </Alert>
                    ...
                    <Box mt={6} display="flex" justifyContent="flex-end">
                        <Button type="submit" variant="contained" disabled={processing} sx={{ px: 10, py: 1.5, borderRadius: 20, bgcolor: 'primary.main', color: 'primary.contrastText', fontWeight: 'bold' }}>Save Everything</Button>
                    </Box>
                </form>
            </Paper>
        </Box>
