...
export default function SellerShipmentsIndex({
    ...
    const totalCbm = manifest_items.reduce((sum, item) => {
        return sum + item.cbm * (quantities[item.id] / item.quantity);
    }, 0);
    const totalKg = manifest_items.reduce((sum, item) => {
        return sum + item.weight_kg * (quantities[item.id] / item.quantity);
    }, 0);
    ...
    const handleDispatch = () => {
        router.post(route("seller.shipments.index"), {
    ...
    return (
        <Box>
            ...
            {/* ── Mobile-first Header ── */}
            <Box sx={{ px: 2, pt: 2, pb: 1, position: "sticky", top: 0, zIndex: 10, backdropFilter: "blur(12px)", bgcolor: "background.default" }}>
                <Stack direction="row" justifyContent="space-between" alignItems="center">
                    <Box>
                        <Stack direction="row" alignItems="center" spacing={0.75}>
                            <LocalShippingIcon color="primary" sx={{ fontSize: 20 }} />
                            <Typography variant="h6" fontWeight={800} sx={{ fontSize: 18 }}>Replenish / Shipments</Typography>
                        </Stack>
                        <Typography variant="caption" color="text.secondary">Phase 1 / 3 — Manifest Configuration</Typography>
                    </Box>
                    <Chip icon={<SyncAltIcon sx={{ fontSize: 12 }} />} label="ONLINE" color="primary" size="small" sx={{ fontWeight: 700, fontSize: 10 }} />
                </Stack>
            </Box>
            ...
            <Box sx={{ px: 2, pb: 2 }}>
                    ...
                    <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ bgcolor: "action.hover", borderRadius: "10px", p: 1.25 }}>
                        <Stack direction="row" alignItems="center" spacing={1}>
                            ...
                            <Box>
                                ...
                                <Typography variant="subtitle2" fontWeight={700} sx={{ fontSize: 13 }}>{scheduled_run}</Typography>
                            </Box>
                        </Stack>
                                    ...
                                    <Stack direction="row" justifyContent="space-between" alignItems="flex-start" spacing={1.25}>
                                        <Stack direction="row" spacing={1.25} sx={{ minWidth: 0 }}>
                                            ...
                                            <Box sx={{ minWidth: 0 }}>
                                                ...
                                                <Typography variant="caption" color="text.secondary" sx={{ fontFamily: "monospace", fontSize: 11, display: "block" }}>
                                                    Plate: {v.plate}
                                                </Typography>
                                    ...
                                    {isSelected && v.bay && (
                                        <Box sx={{ mt: 1.25, px: 1.25, py: 0.5, bgcolor: "action.hover", borderRadius: 1, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                            ...
                                            <Stack direction="row" alignItems="center" spacing={0.5}>
                                                ...
                                                <Typography variant="caption" color="primary.main" fontWeight={700} sx={{ fontFamily: "monospace", fontSize: 11 }}>
                                                    {v.bay} RESERVED
                                                </Typography>
                                            </Stack>
                                        </Box>
                ...
                <Paper elevation={0} sx={{ p: 2, borderRadius: "16px", border: "1px solid", borderColor: "divider", mb: 1.5 }}>
                    <Stack direction="row" justifyContent="space-between" alignItems="center" mb={1.25}>
                        ...
                        <Chip
                            label={`${cbmPercent <= 40 ? "Under" : cbmPercent <= 75 ? "Optimal" : "Near Cap."} (${cbmPercent}%)`}
                            size="small"
                        ...
                        <Box sx={{ flex: 1, minWidth: 0 }}>
                            <Box mb={1.25}>
                                <Stack direction="row" justifyContent="space-between" alignItems="baseline" mb={0.5}>
                                    ...
                                    <Typography variant="caption" fontWeight={800} sx={{ fontFamily: "monospace", fontSize: 11 }}>
                                ...
                                <Typography variant="caption" color="primary.main" fontWeight={700} sx={{ mt: 0.5, display: "block", fontSize: 11 }}>
                                    {(maxCbm - totalCbm).toFixed(1)} CBM Remaining
                                </Typography>
                            </Box>
                            <Box>
                                <Stack direction="row" justifyContent="space-between" alignItems="baseline" mb={0.5}>
                    ...
                    <Stack direction="row" justifyContent="space-around" textAlign="center">
                        {[
                            { label: "SKUs", value: manifest_items.length },
                ...
                <Box mb={2}>
                    <Stack direction="row" justifyContent="space-between" alignItems="flex-start" mb={1.25}>
                        ...
                        <Button variant="contained" startIcon={<AddIcon />} size="small" sx={{ borderRadius: "8px", textTransform: "none", fontWeight: 700, fontSize: 12 }}>
                            Add Items
                        </Button>
                    </Stack>
                    ...
                    <Stack direction="row" justifyContent="space-between" alignItems="center" mb={1.25}>
                        ...
                        <Tooltip title="Reset quantities to defaults">
                            <Button size="small" startIcon={<DeleteSweepIcon sx={{ fontSize: 14 }} />} color="error" sx={{ textTransform: "none", fontSize: 11 }}
                                    ...
                                    <Stack direction="row" justifyContent="space-between" alignItems="flex-start" spacing={1.25} mb={1.25}>
                                        <Stack direction="row" spacing={1.25} sx={{ minWidth: 0 }}>
                                            ...
                                            <Box sx={{ minWidth: 0 }}>
                                                ...
                                                <Stack direction="row" alignItems="center" spacing={0.5} mt={0.25} flexWrap="wrap">
                                                    ...
                                                    <Chip label={item.status === "oos" ? `${item.status_label} (0)` : `${item.status_label} (${item.stock_qty})`} size="small" color={cfg.chipColor} sx={{ fontWeight: 700, fontSize: 9, height: 18 }} />
                                                </Stack>
                                            </Box>
                                        </Stack>
                                        <Box sx={{ textAlign: "right", flexShrink: 0 }}>
                                            ...
                                            <Typography variant="caption" color="text.secondary" sx={{ fontFamily: "monospace", fontSize: 10 }}>
                                                {(item.cbm * quantities[item.id] / item.quantity).toFixed(1)} CBM
                                            </Typography>
                                        </Box>
                                    </Stack>

                                    <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ bgcolor: "action.hover", borderRadius: "8px", px: 1.25, py: 0.5 }}>
                                        <Stack direction="row" alignItems="center" spacing={0.5}>
                                        ...
                                        <Stack direction="row" alignItems="center" spacing={0.75}>
                                            <IconButton size="small" onClick={() => handleQty(item.id, -1)} sx={{ width: 24, height: 24, bgcolor: "background.paper", border: "1px solid", borderColor: "divider" }}>
                ...
                {/* ── Sticky Execution Bar (above bottom nav) ── */}
                <Paper
                    elevation={4}
                    sx={{
                        position: "sticky",
                ...
                >
                    <Box>
                        <Stack direction="row" alignItems="baseline" spacing={0.5}>
                            ...
                            <Typography variant="subtitle1" fontWeight={700} sx={{ fontSize: 16 }}>{manifest_items.length} SKUs</Typography>
                        </Stack>
                        <Typography variant="caption" color="text.secondary" sx={{ fontSize: 11 }}>
