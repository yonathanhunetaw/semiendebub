            ...
            foreach ($itemData['color_ids'] as $colorId) {
                foreach ($sizes as $sizeId) {
                    ...
                    foreach ($itemData['packagings'] as $index => $pkg) {
                        ...
                        $colorName = ItemColor::find($colorId)?->name ?? 'Color_'.$colorId;
                        $sizeName = $sizeId ? ItemSize::find($sizeId)?->name : 'Null';
                        $packName = ItemPackagingType::find($pkg['id'])?->name ?? $pkg['id'];

                        $key = "{$itemData['name']}-{$colorName}-{$sizeName}-{$packName}";

                        $variantImages = [];
                        $slugItem = Str::slug($itemData['name']);
                        $slugColor = Str::slug($colorName);
                        $slugSize = Str::slug($sizeName);
                        $slugPack = Str::slug($packName);
